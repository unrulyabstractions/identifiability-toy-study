...
from .intervention import Intervention, PatchShape

if TYPE_CHECKING:
    from src.circuit import Circuit

# Keys match torch.nn.init.kaiming_normal_ nonlinearity parameter
...
class MLP(nn.Module):
    ...
    def _init_weights(self):
        ...
        for layer in self.layers:
            ...
            if isinstance(linear, nn.Linear):
                nn.init.kaiming_normal_(linear.weight, nonlinearity=self.activation)
    ...
    def visualize(self, ax=None, circuit=None, activations=None):
        ...
        # Set default values
        if circuit is None:
            from src.circuit import Circuit  # late import to avoid circular deps

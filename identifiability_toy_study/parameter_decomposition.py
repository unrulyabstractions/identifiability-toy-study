...
interpretable components. This module provides a simplified interface.
"""
...
class SimpleDataset:
    """Simple dataset wrapper for SPD training with generate_batch method."""
    ...
    def generate_batch(self, batch_size: int) -> tuple[torch.Tensor, torch.Tensor]:
        """Generate a batch by randomly sampling from the data."""
...
def decompose_mlp(
    ...
    Returns:
        DecomposedMLP containing the decomposition result
    """
    ...
    # Build SPD config with all required fields
    config = Config(
        seed=0,
        ...
        # Module patterns to decompose (each layer's Linear)
        module_info=[
        ...
        # Pretrained model info (MLP from this project)
        pretrained_model_class="identifiability_toy_study.common.neural_model.MLP",
        # Task config for TMS-style data generation
        task_config={
            "task_name": "tms",
    ...
    # Freeze target model parameters (required by SPD)
    for param in target_model.parameters():
        param.requires_grad = False

    # Create ComponentModel directly (optimize creates its own but doesn't return it)
    ...
    # Run optimization with a temp output directory
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Load the final checkpoint into our component model
    ...
    return DecomposedMLP(component_model=component_model, target_model=target_model)

...
def generate_trial_data(
    ...
    if logger is not None and debug:
        logger and logger.info(
            f"checking_device: x_train: {train_data.x.device}, y_train: {train_data.x.device}"
        )
        logger and logger.info(
            f"checking_device: x_val: {val_data.x.device}, y_val: {val_data.x.device}"
        )
        logger and logger.info(
            f"checking_device: x_test: {test_data.x.device}, y_test: {test_data.x.device}"
        )
